...
public interface Pitch {
	 ...
	 */
	public boolean isBarEmpty(Spieler player);
	 ...
	 * @param start index of start position, can be negative if start is bar
	 * @param destination index of destination position, can be negative if destination is bar or exit
	 * @param player current player
	 ...
	 */
	public boolean indexInHome(int i, Spieler current);
	
	/**
	 * returns true if current player has all tokens in home indices, so player can now remove tokens
	 * @param current current player
